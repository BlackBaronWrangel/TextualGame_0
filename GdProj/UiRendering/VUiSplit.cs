...
public partial class VUiSplit : VSplitContainer
{
    ...
    private void _on_Container_Dragged(long offset)
    {
        ...
        double newRatio = (offset - ((Control)GetChild(0)).GetMinimumSize().Y) / (totalHeight - ((Control)GetChild(0)).GetMinimumSize().Y - ((Control)GetChild(1)).GetMinimumSize().Y);
    ...
    private void SetSplitRatio(double ratio)
    {
        ...
        var minSizeFirst = ((Control)GetChild(0)).GetMinimumSize().Y;
        var minSizeSecond = ((Control)GetChild(1)).GetMinimumSize().Y;

        // Calculate the total usable size minus the minimum size required for both children
        var totalUsableSize = Size.Y - minSizeFirst - minSizeSecond;
